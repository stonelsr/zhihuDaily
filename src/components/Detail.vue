...
export default {
  ...
  created () {
    api.detail(this.$route.query.detailId).then(({status, data}) => {
      if (status === 200) {
        let {body, title, css,image} = data
        let imgIndex = body.indexOf('img-place-holder"')
        if(imgIndex !== -1) {
          body = `${body.slice(0,(imgIndex+17))} style="background:url(${image});background-size: cover;" ${body.slice((imgIndex+17))}`
        }
        this.body = body
        this.title = title
        this.css = css[0]

        if (document.getElementById('DetailCss') === null) {
          ...
          document.getElementById('DetailCss').href = css;
        }
      }
    })
  }
